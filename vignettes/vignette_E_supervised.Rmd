...
Here, the Soft-Threshold version is used (Truncation version also available) with parameter _shrink = 0.6_ means
...
One set of loadings is printed and two sets are plotted to show how to select specific components from specific blocks. When extracting or plotting loadings for the second or later blocks, one must specify how many components have been used in the previous block as this will affect the choice of loadings.
